...
- 67fc1ed: bash sync→async promotion (`promoteAfterMs`, default off): when configured, bash
...
- 646d5d4: fix(sandbox-e2b): report non-zero shell exits as errors, and stop losing exit codes and output streams
...
- 55f5f0f: Pin the transitive `e2b` SDK to 2.32.0 (exact).
...
- 78b5c47: Carry `config.env` as the per-command environment base on every command
